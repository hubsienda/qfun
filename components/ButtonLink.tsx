...
export function ButtonLink({
  ...
  return (
    <Link
      {...props}
      style={forcedStyle}
      className={`qoobix-focus-ring inline-flex min-h-11 items-center justify-center rounded-full border px-5 py-3 text-sm font-bold tracking-[-0.01em] transition duration-200 ${variantClassNames[variant]} ${className}`}
    >
      {children}
    </Link>
  );
